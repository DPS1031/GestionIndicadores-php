...
switch ($boton) {
    case 'Guardar':
		...
		$datosSubSeccion = ['id' => $id, 'nombre' => $nombre];
		$objSubSeccion= new Entidad($datosSubSeccion);
		$objControlSubSeccion = new ControlEntidad('SubSeccion');
		$objControlSubSeccion->guardar($objSubSeccion);
		header('Location: vistaSubSeccion.php');
		break;

    case 'Consultar':
		$datosSubSeccion=['id' => $id];
		$objSubSeccion = new Entidad($datosSubSeccion); 
		$objControlSubSeccion = new ControlEntidad('SubSeccion');
		$objSubSeccion = $objControlSubSeccion->buscarPorId('id', $id);
		if ($objSubSeccion !== null) {
			$nombre = $objSubSeccion->__get('nombre');
		} else {
    ...
    case 'Modificar':
        ...
        $objSubSeccion=new Entidad($datosSubSeccion);
        $objControlSubSeccion = new ControlEntidad('SubSeccion');
        $objControlSubSeccion->modificar('id', $id, $objSubSeccion);
		header('Location: vistaSubSeccion.php');
        break;
    case 'Borrar':
        $datosSubSeccion=['id' => $id];
        $objSubSeccion = new Entidad($datosSubSeccion);
        $objControlSubSeccion= new ControlEntidad('SubSeccion');
        $objControlSubSeccion->borrar('id', $id);
		header('Location: vistaSubSeccion.php');
        break;
	...
	<div class="table-responsive">
		<div class="table-wrapper">
			<div class="table-title">
				<div class="row">
					<div class="col-sm-6">
						<h2 class="miEstilo">Lista <b>SubSecciones</b></h2>
					</div>
					<div class="col-sm-6">
						<a href="#crudModal" class="btn btn-primary" data-toggle="modal"><i class="material-icons">&#xE84E;</i> <span>Gestión de SubSecciones</span></a>
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td><?php echo $arregloSubSeccion[$i]->__get('id');?></td>
							<td><?php echo $arregloSubSeccion[$i]->__get('nombre');?></td>
							<td>
		...
		<div class="modal-content">
			<form action="vistaSubSeccion.php" method="post">
				<div class="modal-header">						
					<h4 class="modal-title">Sub Seccion</h4>
				...
				<div class="modal-body">
						...
						<ul class="nav nav-tabs" role="tablist">
							<li class="nav-item">
							<a class="nav-link active" data-toggle="tab" href="#home">Datos de Sub Secciones</a>
							</li>
						</ul>
						<!-- Tab panes -->
