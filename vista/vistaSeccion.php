...
switch ($boton) {
    case 'Guardar':
		...
		$datosSeccion = ['id' => $id, 'nombre' => $nombre];
		$objSeccion= new Entidad($datosSeccion);
		$objControlSeccion = new ControlEntidad('Seccion');
		$objControlSeccion->guardar($objSeccion);
    ...
    case 'Consultar':
		...
		$objSeccion = new Entidad($datosSeccion); 
		$objControlSeccion = new ControlEntidad('Seccion');
		$objSeccion = $objControlSeccion->buscarPorId('id', $id);
		if ($objSeccion !== null) {
			$nombre = $objSeccion->__get('nombre');
		} else {
    ...
    case 'Modificar':
        ...
        $objSeccion=new Entidad($datosSeccion);
        $objControlSeccion = new ControlEntidad('Seccion');
        $objControlSeccion->modificar('id', $id, $objSeccion);
		header('Location: vistaSeccion.php');
        break;
    case 'Borrar':
        $datosSeccion=['id' => $id];
        $objSeccion = new Entidad($datosSeccion);
        $objControlSeccion= new ControlEntidad('Seccion');
        $objControlSeccion->borrar('id', $id);
		header('Location: vistaSeccion.php');
        break;
	...
	<div class="table-responsive">
		<div class="table-wrapper">
			<div class="table-title">
				<div class="row">
					<div class="col-sm-6">
						<h2 class="miEstilo">Lista <b>Secciones</b></h2>
					</div>
					<div class="col-sm-6">
						<a href="#crudModal" class="btn btn-primary" data-toggle="modal"><i class="material-icons">&#xE84E;</i> <span>Gestión Secciones</span></a>
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td><?php echo $arregloSeccion[$i]->__get('id');?></td>
							<td><?php echo $arregloSeccion[$i]->__get('nombre');?></td>
							<td>
			...
			<form action="vistaSeccion.php" method="post">
				...
				<div class="modal-body">
						...
						<ul class="nav nav-tabs" role="tablist">
							<li class="nav-item">
							<a class="nav-link active" data-toggle="tab" href="#home">Datos de Secciones</a>
							</li>
						</ul>
						<!-- Tab panes -->
